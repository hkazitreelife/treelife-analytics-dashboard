...
import {
  ...
  isBillingOrTierRejection,
} from "./gemini";

// Re-exported for this worker's callers (processors/documentIngestion.ts):
// the actual definitions live in gemini.ts, since the failure modes are
// identical to the table path's, and reused rather than duplicated.
export { GeminiBillingError, GeminiError, GeminiValidationError };

/**
 * Section 10.0. Extraction for a PDF/PPTX/DOCX, structurally different from
 * gemini.ts's spreadsheet path: there is no deterministic parser that can
 * read PDF/PPTX/DOCX bytes into rows first, so Gemini reads the raw file
 * directly (multimodal inlineData) rather than a JSON preview-rows payload.
 * Gemini is still extraction-only here, exactly as everywhere else in this
 * pipeline: it classifies documentKind and, for a narrative document,
 * returns its text structure -- it never judges importance, never writes a
 * summary, never decides what matters. That is Claude's job
 * (claudeDocumentSummary.ts), same two-model split as the table path.
 *
 * Error classes and the billing-rejection detector are imported from
 * gemini.ts rather than duplicated: the failure modes (billing/tier
 * rejection, empty response, invalid JSON, schema violation) are identical,
 * and a caller that already knows how to handle GeminiValidationError for
 * the table path handles this path's failures the same way.
 */
