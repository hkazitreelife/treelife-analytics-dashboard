...
- **Per-dataset lock built (Section 11.4), commit `9a1aebd`.** `worker/src/services/datasetLock.ts` exports `createDatasetLock(redis)`, sharing the worker's existing Redis connection rather than opening a second one. `acquireLock`: `SET dataset-lock:<datasetId> <token> PX 300000 NX`. `releaseLock`: Lua `GET`-compare-`DEL`, never a bare `DEL`, so a job can only release a lock it still holds. Wired into `processIngestionJob` right after `datasetId` resolves and before any Gemini/Claude call; on contention the job is requeued via BullMQ with a delay instead of being failed, and the rest of the pipeline runs inside a `try/finally` that always releases. Live-tested against a real dataset with worker concurrency briefly raised to 2: contention was correctly detected and requeued without failing the job, and the Redis key's disappearance was confirmed directly (not just inferred from logs) at the moment the holding job's `finally` ran. Worker concurrency stays at 1 in the checked-in config; this lock is what actually protects against two jobs writing to the same Dataset if that's ever raised.
- **Environment hazard, resolved but worth remembering: never run two checkouts of this repo against the same Postgres/Redis.** A stray second checkout (`...\Desktop\claude dekstop dashboard - Copy`) was found running its own `pnpm dev` and `pnpm worker` against the *same* Docker containers as the main checkout, discovered only because it produced a confusing BullMQ-internal failure (`Lock mismatch ... moveToDelayed from active`) during the lock's live contention test. The dataset lock itself was not at fault — verified independently via Redis. The stray checkout and its processes have been deleted. If a second worker or dev server is ever intentionally run against this stack again (a second clone, a worktree, a CI job), point it at separate Postgres/Redis instances, not these shared containers.
...
1. **Config-quality problem, needs a decision before Phase 4 widget/config logic gets extended.** Claude's generated config faithfully renders what it's told to render, but two of its own choices are semantically meaningless even though arithmetically correct: summing a heterogeneous key-value `Constants` table's `value` column (target + tail count + client count + billing fractions, all added together = 128.87, meaningless), and summing `annual_revenue_Cr` across Bands rows that include non-data footer rows. Claude's own insights correctly flag the underlying data problems, but the widgets still show the bad numbers. The Section 15 config contract has no concept of "this table shouldn't be summed" or row-level filtering. Needs either a config-time rule (e.g. `tableRole: config` tables get restricted aggregation options) or a filter concept added to the contract. Not fixed yet, deliberately deferred, but should not keep being deferred indefinitely. Note: a related but distinct bug (a KPI silently fabricating a number from a non-numeric field via digit-stripping) was fixed in Prompt 7.1 — that fix does not cover this one, since the `Constants.value` column genuinely is numeric-typed, just semantically heterogeneous.
2. **Config version hardcoded to 1, no uniqueness constraint.** ~~Re-ingesting the same dataset creates a second version-1 row...~~ Fixed in Prompt 7.1: `worker/src/processors/ingestion.ts` now queries the max existing version for the dataset and writes `+1`; the config-fetch route sorts `-version,-createdAt` for a deterministic tiebreak against any pre-fix duplicate rows.
3. **Dataset marked `failed` when only config generation fails, not the parse.** If Gemini and merge succeed but Claude's config step fails, the Dataset (which holds perfectly good validated data) is marked `failed` anyway. Prompt 7.1 softened the user-facing impact (the dashboard now renders normally with a failure banner when status is `failed` but good data/config still exist, instead of going blank), but the underlying conflation of "parse failed" and "config generation failed" into one status value is still unresolved. Needs a deliberate decision once SSE surfaces status live to the UI.
4. **Gemini's billing/tier-rejection classifier is string-matching on Google's current error wording**, with no structural error code to key on. Documented with a ten-case test suite as the tripwire; will silently stop working if Google changes their wording. Watch, don't rebuild preemptively.
5. **429 (rate limit) on Claude treated as billing failure, fails fast, no backoff.** Defensible for now given BullMQ's own job-level retry exists at the queue layer, but a proper backoff would serve better under real load.
6. **Drizzle `push: true` can hang the worker on an interactive prompt** if a collection field type change is structurally ambiguous (happened once, changing `generatedBy` from relationship to select). No error surfaces, it just hangs. Worth a CLAUDE.md note to check for ambiguity before running push non-interactively; migrations instead of push would remove the whole class of problem, not done yet.
7. **shadcn/ui was not installed via its CLI.** The CLI is interactive and rewrites tsconfig/next.config/global CSS, risking the working Payload build. Primitives were hand-written in the shadcn idiom on Radix instead. PRD Section 8.1 permits "shadcn/ui or equivalent," so this is compliant, but if the real CLI wiring is wanted later, it needs to be done as an isolated change, not assumed already in place.
...
1. **Confirm the Anthropic key rotation status** (item 9 in Section 5) before doing anything else that calls the Claude API.
...
3. ~~Decide on the per-dataset lock approach before starting Phase 4~~ — done, see Section 4 (`worker/src/services/datasetLock.ts`, commit `9a1aebd`).
4. **Decide on the config-quality / aggregation problem** (item 1 in Section 5) — at minimum, agree on whether `tableRole: config` tables get excluded from sum/avg aggregation by default in Claude's config generation, or whether a filter concept gets added to the Section 15 contract. Still open; not the same issue as the non-numeric-field KPI fix from Prompt 7.1.
5. **Phase 4 in progress**: SSE event stream (Section 18) and config versioning (item 2 in Section 5, config version is no longer hardcoded) are done as of Prompt 4.1/7.1. Prompt-based dashboard editing (Section 13) is still to build, deliberately kept separate since it needs real Claude calls to verify.
