...
**Deployment target, decided 2026-08-22: a self-hosted VPS (`docker-compose.prod.yml`), not Vercel.** Both `POST /api/uploads` branches (dataset and document) enqueue via BullMQ (`enqueueIngestion` / `enqueueDocumentIngestion`) to the one persistent worker process, exactly as this section already describes -- no external job service. An Inngest-based trigger for the dataset branch was tried the same day specifically to work around Vercel having no persistent worker, then reverted once the VPS (which has a real persistent worker) became the actual target; if you find Inngest-related code or an `inngest` dependency again later, that's drift, not the current design.
