...
**Job trigger, as of 2026-08-22: Inngest, not BullMQ, for both datasets and documents.** `POST /api/uploads` (both branches) and `POST /api/uploads/confirm` (the update-existing/create-new collision flow, both branches) all call `inngest.send(...)` (`apps/web/lib/inngest.ts`) instead of BullMQ's `enqueueIngestion`/`enqueueDocumentIngestion` -- those two files, and the underlying BullMQ dataset/document queues, no longer exist in `apps/web`. Inngest calls back into `POST /api/inngest` (`apps/web/lib/inngestFunctions.ts`), which runs `processIngestionDirectly` (datasets) or `processDocumentIngestionDirectly` (`apps/web/lib/directDocumentIngestion.ts`, documents) as a durable background job -- the ingestion logic itself is unchanged, only what triggers and retries it. This needs `INNGEST_SIGNING_KEY` (and `INNGEST_EVENT_KEY` in production) configured via the Inngest dashboard; without it the function endpoints exist but Inngest has nothing authorized to call them.

Document ingestion's extraction logic (Gemini document classification, Claude summary generation) was ported into `apps/web/lib/` (`geminiDocument.ts`, `claudeDocumentSummary.ts`, `documentDetector.ts`) specifically so it could run inside an Inngest function -- these mirror their `worker/src/services/` counterparts and should be kept in sync by hand if one changes, same as `sessionWrapper.ts`/`openRouterClient.ts` already are between the two packages.

The BullMQ-driven `worker` process still exists and still runs `worker/src/index.ts` correctly (fixed earlier the same day) -- nothing about it broke, it's simply no longer what triggers dataset/document ingestion in this deployment. Whether to keep running it at all is now an open question, not answered here.

**Known stale signal:** `GET /api/health/queues` checks for a BullMQ worker heartbeat in Redis and will report "no worker heartbeat found" permanently now that nothing drives ingestion through a persistent worker -- that specific signal is misleading, not a sign of a real problem, until/unless that health check is redesigned around Inngest instead.
...
**Known divergence, as of Prompt 16.0: the live dataset extraction path does not call Gemini at all.** `apps/web/lib/directIngestion.ts`'s `parseWorkbookBuffer` -- the function `processIngestionDirectly` actually calls for XLSX structure extraction, and the live path per the Inngest note above -- is a pure client-side heuristic parser (the `xlsx` npm package plus hand-written header detection). It never calls Gemini, never receives the previewRows exception's bounded model call, and was never wired to. The "Gemini does extraction" description above is the target design and is what `worker/src/services/spreadsheetParser.ts` implements; it is not what runs for a real upload today. Do not assume the previewRows exception, or any Gemini-assisted header detection, is actually protecting a live upload -- verify against `directIngestion.ts` directly.

One direct consequence, found and fixed in production (Prompt 16.0 item 8): the normalized contract's one-`headerRowIndex`-per-table shape assumes one logical table per worksheet. Neither the live heuristic parser nor the documented Gemini design covers a worksheet holding two genuinely separate tables (different shapes, stacked with a blank row between them) as an explicit case -- the contract has no field for "this sheet has more than one table." `parseWorkbookBuffer` now splits a sheet into blocks at blank-row boundaries and only treats two blocks as separate tables when their column counts differ (biased against over-splitting a real table that has a stray blank row or a repeated header for pagination). This is a deterministic heuristic, not model judgment, and has a named limitation: two genuinely different tables that happen to share the same column count still merge into one. Fully closing that would need either comparing real header text (which breaks the legitimate repeated-header-for-pagination case) or actual model-assisted structure detection -- neither exists today.
