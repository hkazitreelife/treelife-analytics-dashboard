...
/**
 * Mirrors worker/src/services/claudeDocumentSummary.ts exactly (including
 * its already-fixed OpenRouter branch). Separate package, can't share the
 * source directly.
 *
 ...
 * decides what matters. Claude never re-extracts and never receives the raw
 * file -- only the already-extracted, already-validated text
 * (geminiDocument.ts's job).
 */
...
export const createClaudeDocumentSummaryClient = (
  ...
  return {
    ...
    generateSummary: async (fullText, sections, options) => {
      ...
      // With an OpenRouter-format ANTHROPIC_API_KEY, calling Claude's
      // native SDK straight at ANTHROPIC_BASE_URL fails outright. The
      // prompt schema is serialized from documentSummaryToolSchema
